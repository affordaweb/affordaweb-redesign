...
const GEMINI_KEY_PATH = path.join(__dirname, '..', 'gemini-service-account-key.json');
const HAS_GEMINI_KEY = fs.existsSync(GEMINI_KEY_PATH);

const SITES = {
  'affordaweb': {
    domain: 'sc-domain:affordawebsolutions.com',
    displayUrl: 'https://affordawebsolutions.com',
    ga4PropertyId: null,
  },
  'ascend': {
    domain: 'https://ascendevsolutions.com',
    ga4PropertyId: null,
  },
  'silbi-batangas': {
    domain: 'https://silbibatangas.org',
    ga4PropertyId: null,
  },
  'wagayway': {
    domain: 'https://wagaywayequality.org',
    ga4PropertyId: null,
  },
  'lgbt-ku-ila': {
...
async function getGeminiInsight(siteUrl) {
  try {
    let token;
    const geminiClient = await getGeminiAuth();
    if (geminiClient) {
      const t = await geminiClient.getAccessToken();
      token = t.token;
    } else {
      const apiKey = process.env.GEMINI_API_KEY || process.env.PAGESPEED_API_KEY;
      if (!apiKey) return { error: 'No Gemini auth available' };
...
async function analyzeSite(siteKey) {
  ...
  const [gscData, speedData] = await Promise.all([
...
async function main() {
  ...
  if (command === 'analyze') {
    ...
    if (!target) {
      console.log('Usage: node scripts/google-services.js ai <siteKey>');
      console.log(`Sites: ${Object.keys(SITES).join(', ')}`);
      return;
    }
    const site = SITES[target];
    ...
    const result = await getGeminiInsight(site.displayUrl || site.domain);
    if (result && !result.error) {
      console.log(`\n=== AI SEO Insights for ${target} ===`);
      console.log(result);
    } else {
      console.log('AI insight unavailable:', result?.error || 'Unknown error');
    }
...
`);
    ...
    console.log(`
Usage:
  node scripts/google-services.js analyze [site|all]    - GSC + PageSpeed data
  node scripts/google-services.js ai <siteKey>           - AI SEO insights (uses Gemini, 20/day)
